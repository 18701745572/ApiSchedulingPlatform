...
require('../Comment.php');
if($ulogin['status']!=1||$ulogin['status']==""||$ip!=$ulogin['ip']){};
if($ulogin['status']==1)exit("<script language='javascript'>window.location.href='../user';</script>");
?>
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        ...
        .login-main .login-bottom .tip .icon-nocheck {display:inline-block;width:10px;height:10px;border-radius:2px;border:solid 1px #9abcda;position:relative;top:2px;margin:1px;cursor:pointer;}
        .login-main .login-bottom .tip .icon-check {margin:0;width:14px;height:14px;border:none;background:url(undefinedcss/images/icon-login.png) no-repeat -111px -47px;}
        ...
        .login-main .login-bottom .tip .forget-password {font-stretch:normal;letter-spacing:0;color:#1391ff;text-decoration:none;}
        ...
        <form class="layui-form login-bottom">
            ...
            <div class="tip">
                ...
                <span style="float:right">
                    <a href="register.php" class="forget-password" style="right:130px;">没有账号</a>
                    <span>|</span>
    ...
    layui.use(['form','jquery'], function () {
        ...
        $('.login-tip').on('click', function () {
            if ($('.icon-nocheck').hasClass('icon-check')) {
                $('.icon-nocheck').removeClass('icon-check');
            } else {
        ...
        form.on('submit(login)', function (data) {
	        ...
	        $.ajax({
		        ...
		        success : function(data) {
			        ...
			        if(data.code == 0){
				        layer.msg(data.msg);
				        setTimeout(function(){window.location = '../user';},800);
			        }else{
			            layer.msg(data.msg);
